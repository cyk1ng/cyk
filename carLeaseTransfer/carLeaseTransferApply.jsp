...
<body>
	<div class="mini-fit">
		<div id="tabs1" class="mini-tabs" activeIndex="0" style="width:100%;height:100%;" plain="false" buttons="#tabsButtons" buttonsAlign="right">
		    <!-- START 申请信息 -->
			<%@ include file="carLeaseTransferApply_applyInfo.jsp" %>
			<!-- END 申请信息 -->
			<!-- START 业务信息 -->
			<%@ include file="carLeaseTransferApply_businessInfo.jsp" %>
			<!-- END 业务信息 -->
			<!-- START 申请材料 -->
			<%@ include file="carLeaseTransferApply_applyMaterial.jsp" %>
			<!-- END 申请材料 -->
			<!-- START 法律文书 -->
...
function setCompanyQuota(enterpriseId) {
	...
	$.ajax({
		type : "POST", 
		url : "${webProperties['app.url.approval']}/carLeaseTransfer/getCarCreditData.htm",
		data : {  
...
function getIllegalCarInfo(enterpriseNum) {
	$.ajax({
        url: "${webProperties['app.url.approval']}/carLeaseTransfer/queryIllegalCarInfoAllByCom.htm",
        data:{
...
<script type="text/javascript">
	...
	$(function() {
		...
		try {if ("${taxiBussinessEntity.projectnameSix}" == null || "${taxiBussinessEntity.projectnameSix}" == "") {mini.get("#projectnameSix").setValue(window.parent.itemId);}} catch(e) {mini.get("#projectnameSix").setValue("${taxiBussinessEntity.projectnameSix }");console.log(e);}
...
// 		isViewCarList();
		...
		if("1" != type && "4" != type){
			...
			try {getHistoryTasksByCondition({instanceId:window.parent.processInstanceId},dataObj);} catch(e) {console.log(e);}
		}
	...
	function getRequestString(key){
  	...
  	function doSave(){
		...
		if (applyName == '' || applyName == null) {
  		...
  		loading(true);
		$.ajax({
	       	url: "${webProperties['app.url.approval']}/carLeaseTransfer/saveCarLeaseTransfer.htm",
			type: 'post',
	        contentType: "application/x-www-form-urlencoded; charset=utf-8",
			data:{
	...
	function doAccept(){
		...
		mini.confirm("确认要办结吗？", "确定", function(action) {
			if (action == "ok") {
				...
				loading(true);
				$.ajax({
			       	url: "${webProperties['app.url.approval']}/carLeaseTransfer/acceptCarLeaseTransfer.htm",
					type: 'post',
			        contentType: "application/x-www-form-urlencoded; charset=utf-8",
					data:{
						data:mini.encode(formData),
	...
	function doNotAccept(){
		...
		mini.confirm("确认要不予受理吗？", "确定", function(action){
			if (action == "ok") {
				...
				loading(true);
				$.ajax({
			       	url: "${webProperties['app.url.approval']}/carLeaseTransfer/noAcceptCarLeaseTransfer.htm",
					type: 'post',
			        contentType: "application/x-www-form-urlencoded; charset=utf-8",
					data:{
						data:mini.encode(formData),
...
//         if (form_apply.isValid() == false || form_company.isValid() == false || form_quota.isValid() == false){
       	if (form_apply.isValid() == false || form_company.isValid() == false){
...
// 		Object.assign(formData, form_applyData, form_companyData, form_quotaData, form_hiddenData); // 多个form合拼成一个
		Object.assign(formData, form_applyData, form_companyData, form_hiddenData); // 多个form合拼成一个
...
// 		}
		...
		for (var i = 0; i < vehiclesData.length; i++) {
			vehiclesData[i].operationCertificateClass = "2"; // 租赁固定传2
			// 车辆类型没有值时
...
// 		if (carInfoRdio == "1") {
		    ...
		    if (vehicles != null && vehicles.length > 0) {
				for (var i = 0; i < vehicles.length; i++) {
					...
					} else if (vehicles[i].carType == "4") {
						mediumCarQuota = mediumCarQuota + 1;
					}  else if (vehicles[i].carType == "6") {
