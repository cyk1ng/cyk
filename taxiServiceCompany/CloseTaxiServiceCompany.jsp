...
<script type="text/javascript">
	...
	$(document).ready(function(){
        ...
        if("view" == model){
        	...
        	try {window.parent.setHangYe("出租","2011","","2");} catch(e) {console.log(e);}
        }
...
//                 mini.alert("当场办结成功");
                ...
                //调用制证
                operCert(result.acceptNumber,'taxi');
            ...
            success:function(data){
                ...
                if (dataObj.data.length>0) {
                    ...
                    if (eduFlag == 2) {
                        ...
                        mini.confirm("还有剩余额度，是否继续不予受理？", "确定？",
                            function (action) {
                                if (action == "ok") {
                                	buyushouli();
                                } else {
    ...
    function eduValid() {
        var url = "${webProperties['app.url.approval']}/taxiServiceComp/getEDuData.htm";
        var enterpriseId = mini.get("#ownerId").getValue();
        $.ajax({
            url:url,
            data:{
