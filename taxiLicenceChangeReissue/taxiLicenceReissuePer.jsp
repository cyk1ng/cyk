    ...
    <div id="tabs1" class="mini-tabs" activeIndex="0" style="width:100%;height:100%;" plain="false"
        ...
        <div title="申请信息">
            ...
            <form id="hiddenDomainForm" method="post">
                ...
                <input id="companyEconomyType" name="companyEconomyType" style="display:none" class="mini-hidden"
                       value=""/>
            ...
            <form id="applicationForm" method="post">
                <table border="0" cellpadding="1" cellspacing="2"
                    ...
                    <tr>
                        <td style="width:130px;" align="right">事项类别：</td>
                        <td style="width:100%" colspan="3">
                            <div id="projectnameSix" name="projectnameSix" class="mini-checkboxlist" textField="text"
                                 valueField="id" value="089106004001" onvaluechanged="changedProjectValue();"
                                 required="true" requiredErrorText="请至少选择一项业务事项">
                            </div>
                ...
                <form id="businessForm">
                    ...
                    <div style="width: 48%;height: 100%;float: left;">
                        <div id="datagridCar" class="mini-datagrid" idField="id"
                             url="${webProperties['app.url.approval']}/taxi/queryTaxiLicenceReissueCarList.htm"
...
<script type="text/javascript">
    ...
    var businessItemType = [
        {"id": "089106004001", "text": "补正证"},
        {"id": "089106004002", "text": "补副证"}
    ];
...
//         }
        ...
        if ("" != acceptNumber && null != acceptNumber) {
            ...
            $("#fileupload").load(function () {
                $("#fileupload")[0].contentWindow.getList("089106004001");
            })
        }
        getApprovalType({});
    ...
    function changedProjectValue() {
        ...
        $("#fileupload")[0].contentWindow.getList(mini.get('projectnameSix').getValue().toString());
    }
     ...
     */
    function submitSL() {
        var url = "${webProperties['app.url.approval']}/taxi/taxiLicenceReissueSL.htm";
        operating(url, true, 1, "确定当场办结吗？")
    }
     ...
     */
    function submitNoSL() {
        var url = "${webProperties['app.url.approval']}/taxi/taxiLicenceReissueNoSL.htm";
    ...
    function submitSave() {
        if (type == "2" && model == "view") {
            ...
            //保存附件信息
            $.ajax({
                url:  "${webProperties['app.url.approval']}/taxi/taxiSaveFileInfo.htm",
                data:{
            ...
            var url = "${webProperties['app.url.approval']}/taxi/taxiLicenceReissueSave.htm";
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (type == 1) {
                        ...
                        var pgxxArr = Obj.projectnameSix.split(",");
                        var businessMatters = [];
                        for (var i = 0; i < pgxxArr.length; i++) {
                            //补正证
                            if (pgxxArr[i] == "089106004001") {
                                businessMatters.push("补正证")
                            }
                            //补副证
                            if (pgxxArr[i] == "089106004002") {
                                businessMatters.push("补副证")
                            }
                        }
                        $("#businessMatters").css("display", "block");
                        mini.get("#businessMatters").setValue(businessMatters.toString());
                        //制证
                        createVehicleOperCert(dataObj.acceptNumber,"taxi");
    ...
    function queryTaxiBusinessLicense(businessSearchObj) {
        loading(true);
        $.ajax({
            url: "${webProperties['app.url.approval']}/taxi/queryTaxiReissueInfo.htm",
            type: 'post',
            contentType: "application/x-www-form-urlencoded; charset=utf-8",
