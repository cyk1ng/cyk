...
<script type="text/javascript">
        ...
        $.ajax({
            ...
            success: function (debugData) {
                ...
                for (var i=0;i<entranceName_.length;i++){
                    entranceDom+="<input  value='"+entranceName_[i]+"' readonly class=\"form-control asLabel\"  vtype=\"rangeChar:1,132\" />"
                }
                for (var j=0;j<exitName_.length;j++){
                    exitDom+="<input  value='"+exitName_[j]+"' readonly class=\"form-control asLabel\"  vtype=\"rangeChar:1,132\" />"
                }
    ...
    function submit(acceptType){
        ...
        console.log(json);
        getAcceptNumber(data);
        //调试使用
        //data=debugData;
        function getAcceptNumber(formData) {
            $.ajax({
                url: "${webProperties['app.url.commProcess']}/entrance/getAcceptNumber.htm",
                type: 'get',
                contentType: "application/json;charset=utf-8",
                data:{itemId: formData.ckType,type:'02'},
                success: function(data){
                    //插入受理编号
                    $.ajax({
                        url: "${webProperties['app.url.businessApproval.netWeb']}/park/NewParkApproval.data?acceptNumber="+data.replace(/\"/g,""),
                        contentType : "application/json;charset=utf-8",
                        type : "POST",
                        dataType : "json",
