		    	...
		    	<form id="form" method="post">
		    		<table border="0" cellpadding="1" cellspacing="2" style="width:100%;table-layout:fixed;line-height: 40px;padding-right:40px;">
					    ...
					    <tr>
					       	...
					       	<td style="width:100%" colspan="3"> 
							<div class="mini-radiobuttonlist" id="projectnameSix" name="projectnameSix" repeatItems="1" repeatLayout="table" required="true" repeatDirection="vertical" textField="text" valueField="id" onvaluechanged= "projectnameSixChange()"  data="[{id:'089203001',text:'换证'},{id:'089203002',text:'补证'}]" value="${taxiBussinessEntity.projectnameSix}"></div>
							</td>
				       	</tr>
					    <tr>
							<td>
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		if("view" == model){
			...
			try {window.parent.setHangYe("租赁","2012","","");} catch(e) {console.log(e);}
		}
	...
	function saveTaxiLeaseRenew(){
		...
		$.ajax({
	       	url: "${webProperties['app.url.approval']}/carLeaseRenew/saveCarLeaseRenew.htm",
			type: 'post',
		...
		mini.confirm("确定办结？", "确定？",
			function (action) {
				if (action == "ok") {
					...
					$.ajax({
				       	url: "${webProperties['app.url.approval']}/carLeaseRenew/sceneCarLeaseRenew.htm",
						type: 'post',
						data:{
...
// 			                mini.alert("办结成功");
	...
	//不予受理
	function noAcceptTaxiLeaseRenew(){
		var applyName = mini.get("applyName").getValue();
		if (applyName == '' || applyName == null) {
			mini.alert("企业申请信息不能为空！");
			return;
		...
		mini.confirm("确定不予受理？", "确定？",
			function (action) {
				if (action == "ok") {
					...
					$.ajax({
				       	url: "${webProperties['app.url.approval']}/carLeaseRenew/noAcceptCarLeaseRenew.htm",
						type: 'post',
						data:{
			                data:mini.encode(obj)
						},
