...
<body>
	<div class="mini-fit">
		<div id="tabs1" class="mini-tabs" activeIndex="0" style="width:100%;height:100%;" plain="false" buttons="#tabsButtons" buttonsAlign="right">
		    <!-- START 申请信息 -->
		    <%@ include file="networkReservationTaxiOp_applyInfo.jsp" %>
		    <!-- END 申请信息 -->
		    <!-- START 业务信息 -->
			<%@ include file="networkReservationTaxiOp_businessInfo.jsp" %>
			<!-- END 业务信息 -->
			<!-- START 申请材料 -->
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		if (pageType == 'perTaxiAdd' || pageType == 'perTaxiDel' || pageType == 'perTaxiLicense') {
			...
			gridCarList.hideColumn(gridCarList.getColumn(2));
			gridNewCarList.hideColumn(gridNewCarList.getColumn(3));
		}
        ...
        if("view" == model){
			...
			window.parent.setHangYe("网约","2011","","");
		}
		// 初始化右表数据
		 initGridData();
		//初始化左表数据
...
// 					initLeftGridData();
	...
	function doSave(){
		...
		$.ajax({
	       	url: "${webProperties['app.url.approval']}/networkReservationTaxiOp/saveNetworkTaxiOp.htm",
			type: 'post',
	...
	function scene(){
		...
		$.ajax({
	       	url: "${webProperties['app.url.approval']}/networkReservationTaxiOp/sceneNetworkTaxiOp.htm",
			type: 'post',
	        contentType: "application/x-www-form-urlencoded; charset=utf-8",
			data:{data:mini.encode(businessObj), vehicles:mini.encode(vehicles)},
			success:function(data){
...
//                 mini.alert("办结成功");
	...
	function noAccept(){
		...
		$.ajax({
	       	url: "${webProperties['app.url.approval']}/networkReservationTaxiOp/noAcceptNetworkTaxiOp.htm",
			type: 'post',
	        contentType: "application/x-www-form-urlencoded; charset=utf-8",
			data:{data:mini.encode(businessObj), vehicles:mini.encode(vehicles)},
			success:function(data){
