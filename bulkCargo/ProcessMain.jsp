...
<script type="text/javascript">
    ...
    function queryTaxiBusinessLicense(businessSearchObj) {
        $.ajax({
            url: businessApprovalUrl+"/common/queryBusinessLicense.htm",
            type: 'post',
            ...
            success: function (data) {
                ...
                businessObj = dataObj;
                console.log("----------------query approval---------------");
                console.log(businessObj);
                $("#acceptTime").html(mini.formatDate(dataObj.acceptTime, "yyyy-MM-dd"));
                $("#acceptUser").html(dataObj.acceptUser);
                $("#acceptNumber").html(dataObj.acceptNumber);
                $("#processStatus").html(processStatus);
                queryItemInfo(dataObj.applicationClass);

                var defineKey = '${defineKey}';

                if (defineKey == "usertask3") {
                    var projectnameSixArr = dataObj.projectnameSix.split(",");
                    for (var i = 0; i < projectnameSixArr.length; i++) {
                        if (jQuery.inArray(projectnameSixArr[i], fieldAuditProjectNameSixArr) >= 0) {
                            matterType = "2";
                            break;
                        } else {
                            matterType = "1";
	...
	function getHistoryTasksByCondition(paramObj){
		...
		$.ajax({
			...
			success:function(data){
				...
				if(("usertask22" == taskDefinitionKey || "usertask31" == taskDefinitionKey) && "2" == type){
					var taskObj = result[1];
					jQuery("#appOpinion_span_id").text(taskObj.taskLocalVariables.appOpinion);
					mini.get("appResult").setValue(taskObj.taskLocalVariables.appResult);
					if(taskObj.taskLocalVariables.appResult == "1"){
						jQuery("#appResult_span_id").append("<img src='${webProperties["app.url.businessApproval"]}/resources/approve.png'/>");
					}else{
						jQuery("#appResult_span_id").append("<img src='${webProperties["app.url.businessApproval"]}/resources/disapprove.png'/>");
					}
				}
